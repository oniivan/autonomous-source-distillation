# Case D: Source Composition, Order A
...
Build a schema-v3 serious bundle in `output/`. Copy all input files into
`output/inputs/`, retain the declared source IDs, and preserve source-family and
derivation metadata from `source-registry.json`. Run the bundle auditor with
`--require-ready`.
...
{
  "schema_version": 1,
  "case_id": "D-source-composition-order-a",
